    ...
    const fetchTransactions = async () => {
      if (window.ethereum) {
        try {
          ...
          const response = await fetch(`https://explorer.celo.org/alfajores/api/?module=account&action=txlist&address=${userAddress}`);
  ...
  return (
    <div className="container mx-auto p-4 lg:p-0">
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-prosperity shadow-md rounded-lg p-4">
          {transactions && transactions.length > 0 ? (
            transactions.slice(0, 7).map((transaction, index) => (
              <div key={transaction.key} className={`flex flex-row gap-2 items-center justify-between p-4 mb-2 rounded-lg ${index % 2 === 0 ? 'bg-prosperity' : 'bg-prosperity'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a
                    href={`https://explorer.celo.org/alfajores/tx/${transaction.transactionHash}`}
                    target="_blank"
